...
const Game = () => {
  ...
  const [showProfit, setShowProfit] = useState(bet);
  const [showProfit2, setShowProfit2] = useState(bet2);
  ...
  const [playedBet2, setPlayedBet2] = useState(false);
  const [outCash, setOutCash] = useState(false);
  const [outCash2, setOutCash2] = useState(false);
  const [status, setStatus] = useState(null);
  const socketRef = useRef(null);
  const playedBetRef = useRef(null);
  ...
  useEffect(() => {
    playedBetRef2.current = playedBet2;
    betRef2.current = bet2;
  }, [playedBet2])
  ...
  useEffect(() => {
    ...
    socketRef.current.onmessage = (event) => {
      ...
      if (data.type === 'IN_PROGRESS') {
        ...
        let profit = betRef.current * data.multiplier
        setShowProfit(profit);

        let profit2 = betRef2.current * data.multiplier
        setShowProfit2(profit2);
      }
  ...
  const removeTen2 = () => {
    if (bet2 >= 20 && !playedBet2) {
      setBet2(pre => pre - 10);
    }
  }

  const addTen2 = () => {
    if (amount >= 10 && !playedBet2) {
      setBet2(pre => pre + 10);
    }
  }
    ...
    <div className='grid grid-cols-1 place-items-center bg-black/60'>
      <div className='bg-black/80 w-full max-w-[1800px] h-full text-white'>
        ...
        <div className='flex flex-col-reverse lg:flex-row gap-1 mt-1 px-2 min-h-[95vh] mb-3'>
          ...
          <div className='bg-black/40 w-full lg:w-[25vw] py-4 px-2 font-semibold rounded-2xl bet-history'>
                ...
                <div className='flex gap-4 justify-center items-center flex-wrap'>
                  <div className='flex flex-col gap-2'>
                    ...
                    <div className='grid grid-cols-2 gap-x-1 gap-y-1'>
                      <p onClick={() => { !playedBet && setBet(10) }} className='active:scale-95 cursor-pointer hover:bg-gray-600 bg-black/40 text-center px-2 rounded-xl'>10</p>
                      <p onClick={() => { !playedBet && setBet(100) }} className='active:scale-95 cursor-pointer hover:bg-gray-600 bg-black/40 text-center px-2 rounded-xl'>100</p>
                      <p onClick={() => { !playedBet && setBet(500) }} className='active:scale-95 cursor-pointer hover:bg-gray-600 bg-black/40 text-center px-2 rounded-xl'>500</p>
                      <p onClick={() => { !playedBet && setBet(1000) }} className='active:scale-95 cursor-pointer hover:bg-gray-600 bg-black/40 text-center px-2 rounded-xl'>1000</p>
                    </div>
                  </div>
                  <button disabled={!status == 4} onClick={() => makeBet()} className={`${playedBet ? "bg-yellow-400" : "bg-green-600"} cursor-pointer hover:ring-2 ${playedBet ? "ring-yellow-200" : "ring-green-500"} active:scale-95 border rounded-3xl py-4 ${playedBet ? "px-12" : "px-18"} text-2xl font-semibold`}><h1>{`${playedBet ? "CASHOUT" : "BET"}`}</h1><h2>{playedBet && status == 2 ? showProfit.toFixed(2) : bet.toFixed(2)}</h2></button>
                </div>
              </div>
              <div className='flex flex-col gap-4 justify-center items-center bg-gray-600/40 rounded-2xl w-full px-4 py-2'>
                <div className='flex gap-4 px-4 py-1 rounded-xl bg-black/60'>
                ...
                <div className='flex gap-4 justify-center items-center flex-wrap'>
                  <div className='flex flex-col gap-2'>
                    <div className='flex bg-black/60 rounded-xl px-4 py-1'>
                      <button onClick={() => removeTen2()} className='border px-[4px] py-[3px] cursor-pointer bg-black rounded-full'><IoIosRemove /></button>
                      ...
                      <button onClick={() => addTen2()} className='border px-[4px] py-[4px] bg-black cursor-pointer rounded-full'><IoIosAdd /></button>
                    </div>
                    <div className='grid grid-cols-2 gap-x-1 gap-y-1'>
                      <p onClick={() => { !playedBet2 && setBet2(10) }} className='active:scale-95 cursor-pointer hover:bg-gray-600 bg-black/40 text-center px-2 rounded-xl'>10</p>
                      <p onClick={() => { !playedBet2 && setBet2(100) }} className='active:scale-95 cursor-pointer hover:bg-gray-600 bg-black/40 text-center px-2 rounded-xl'>100</p>
                      <p onClick={() => { !playedBet2 && setBet2(500) }} className='active:scale-95 cursor-pointer hover:bg-gray-600 bg-black/40 text-center px-2 rounded-xl'>500</p>
                      <p onClick={() => { !playedBet2 && setBet2(1000) }} className='active:scale-95 cursor-pointer hover:bg-gray-600 bg-black/40 text-center px-2 rounded-xl'>1000</p>
                    </div>
                  </div>
                  <button disabled={!status == 4} onClick={() => makeBet2()} className={`${playedBet2 ? "bg-yellow-400" : "bg-green-600"} cursor-pointer hover:ring-2 ${playedBet2 ? "ring-yellow-200" : "ring-green-500"} active:scale-95 border rounded-3xl py-4 ${playedBet2 ? "px-12" : "px-18"} text-2xl font-semibold`}><h1>{`${playedBet2 ? "CASHOUT" : "BET"}`}</h1><h2>{playedBet2 && status == 2 ? showProfit2.toFixed(2) : bet2.toFixed(2)}</h2></button>
                </div>
              </div>
